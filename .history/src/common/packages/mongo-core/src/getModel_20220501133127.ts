...
export function getModel<U extends AnyParamConstructor<any>, QueryHelpers = AnyObject>(cl: U, options?: IModelOptions | undefined): ReturnModelType<U, QueryHelpers> {
  ...
  for (const { target, propName } of properties) {
    ...
    if (property.type.isClass()) {
      promises.push(property.type.getCtor().then(constructor => {
        const desc = Object.getOwnPropertyDescriptor(Custom.prototype, propName);
        Reflect.decorate([
          // @ts-ignore
          Prop({ type: () => constructor })(Custom.prototype, propName)(Custom.prototype, propName),
  ...
  Promise.all(promises).then(() => {
    ...
    console.log(AdvancedModel.schema.obj)
  })
