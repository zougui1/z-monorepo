...
export const getModel = <U, QueryHelpers = AnyObject>(cl: U, options?: IModelOptions | undefined): ReturnModelType<AnyParamConstructor<any>, QueryHelpers> => {
  const properties = Reflect.getMetadata('custom-properties', cl) as { target: Object; propName: string }[];
  const type = getType<U>();
  //console.log(types);

  const BasicModel = getModelForClass<AnyParamConstructor<any>, QueryHelpers>(cl as any, options);
  let AdvancedModel: ReturnModelType<AnyParamConstructor<any>, QueryHelpers> | undefined;

  const bufferProxyHandler: ProxyHandler<ReturnModelType<AnyParamConstructor<any>, QueryHelpers>> = {
    get(target, propName) {
      const prop = (target as any)[propName];
      console.log({ propName });
  ...
  for (const { target, propName } of properties) {
    ...
    if (!property) {
      throw new Error(`Could not find property "${propName}" on class "${(cl as any).name}"`);
    }
  ...
  Promise.all(promises).then(() => {
    console.log('all')
    AdvancedModel = getModelForClass<AnyParamConstructor<any>, QueryHelpers>(cl as any, options);
  })
