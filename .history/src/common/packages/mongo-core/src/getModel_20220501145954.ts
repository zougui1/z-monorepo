...
export function getModel<U extends AnyParamConstructor<any>, QueryHelpers = AnyObject>(cl: U, options?: IModelOptions | undefined): ReturnModelType<U, QueryHelpers> {
  const properties = (Reflect.getMetadata('custom-properties', cl) || []) as { target: Object; propName: string; isPrimitiveTye: boolean }[];
  ...
  const bufferProxyHandler: ProxyHandler<ReturnModelType<U, QueryHelpers>> = {
    get(target, propName) {
      ...
      console.log('name', (Model as any).schema.obj.name.type.name)
      console.log('name.some', (Model as any).schema.obj.name.type.some.type.name)
      console.log('name.some.number', (Model as any).schema.obj.name.type.some.type.number.type.name)
...
const decorateType = (type: Type, origin: Constructor<unknown>, target: Constructor<unknown>): Promise<void>[] => {
  ...
  const promises: Promise<void>[] = [];
  const typeProperties = type.getProperties();
  const propertiesNames = (Reflect.getMetadata('custom-properties', origin) || []) as { target: Object; propName: string; basicType: Function; isPrimitiveTye: boolean }[];

  for (const { propName, isPrimitiveTye, basicType } of propertiesNames) {
    //console.log({propName})
    const property = typeProperties.find(property => property.name === propName);
    ...
    if (propertyType.isClass()) {
      const promise = new Promise(r => setTimeout(r, 500))
        ...
        .then(async constructor => {
          ...
          await Promise.all(decorateType(propertyType, constructor, target));
        });
