  ...
  for (const { target, propName } of properties) {
    ...
    if (property.type.isClass()) {
      promises.push(property.type.getCtor().then(constructor => {
        console.log(BasicModel.schema.obj.name)
        BasicModel.schema.remove(propName);
        console.log(BasicModel.schema.obj.name)

        console.log('sub props:', constructor?.name, (Reflect.getMetadata('custom-properties', constructor?.prototype) || []) )
