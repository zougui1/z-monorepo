...
export const makeImagePreviews = async <FormatName extends MultiOptimizeSupportedFormats>(
  ...
  const files = formatsEntries.reduce((files, [formatName, formatOptions]) => {
    const output = outputs.find(output => output.format === formatName) as sharp.OutputInfo;
    files[formatName] = {
      format: formatName,
...
export type PreviewsResult<FormatName extends MultiOptimizeSupportedFormats> = {
  [Key in FormatName]: {
    file: string;
    format: Key;
