...
import { Exception, Cause } from './Exceptiopn';
...
> => {
  return class CustomException extends Exception<RequiredErrorData<TData>, RequiredErrorCause<TCause>> {
    ...
    // a type error occurs
    // "Type 'ExceptionUnknownOptions' is not assignable to type 'InferExceptionOptions<ExceptionArguments<TData, TCause>>'"
...
export type ExceptionUnknownOptions = {
...
> = (
  TData extends UnknownObject
    ? TCause extends Cause
      ...
      : [{ data: TData; cause?: Cause | undefined }]
    : TCause extends Cause
      ? [{ data?: UnknownObject | undefined; cause: TCause }]
      : ([ExceptionUnknownOptions] | ExceptionUnknownOptions[])
)
...
export type RequiredErrorCause<TCause> = TCause extends Cause ? TCause : never;
