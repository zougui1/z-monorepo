...
describe('createException()', () => {
  ...
  it('should be an instance of Exception', () => {
    const MyError = createException({ code, message, version });
  ...
  it('should have the correct properties defined', () => {
    const MyError = createException({ code, message, name, version });
    const error = new MyError();
  ...
  it('should give a custom data object', () => {
    ...
    const MyError = createException<{ myString: string }>({ code, message, name, version });
  ...
  it('should give a custom cause', () => {
    ...
    const MyError = createException<void, Error>({ code, message, name, version });
  ...
  it('should take the name of the class when extended', () => {
    class MyError extends createException({ code, message, version }) {}
    const error = new MyError();
...
// test when an exception is supposed to have data only
{
  ...
  // @ts-expect-error when any cause is given
  new MyError({
    cause: new Error() as any,
  });
}
...
// test when an exception is supposed to have both data and a cause
{
  ...
  const MyError = createException<{ myString: string }, Error>({ code, message, name, version });
...
// test when an exception is supposed to have a custom exception as cause
{
  ...
  const SomeError = createException<{ str: string }>({ code, message, name, version });
  const MyError = createException<void, InstanceType<typeof SomeError>>({ code, message, name, version });
