...
> extends Error {
  cause: Exclude<TCause, unknown | undefined>;
  code: string;
  data: TData;
  date = new Date();
  ...
  constructor(error: ExceptionData<TData, TCause>) {
    ...
    this.cause = getCause(error.cause) as Exclude<TCause, unknown | undefined>;
...
const getCause = <TCause extends Cause | unknown | undefined>(error: TCause): Exclude<TCause, unknown | undefined> => {
  if (error instanceof Exception) {
    return error as Exclude<TCause, unknown | undefined>;
  }

  if (isNativeError(error)) {
    return getEnumerableError(error) as Exclude<TCause, unknown | undefined>;
  }
  ...
  return {
    ...
    stack: undefined,
  } as Exclude<TCause, unknown | undefined>;
}
