...
const MyTask = createTaskLogs<{ args: [url: string] }, { id: string }, Error>({
  baseCode: 'my.log',
  namespace: 'zougui:logger',
  messages: {
    start: 'Starting...',
...
const subFn = (num: string): { id: string } => {
  return { id: 'some id' };
}
...
const fn = async () => {
  ...
  logger.info(logs.start({ data: { url: '' } }))
  ...
  const res = MyTask.wrap(faBrowse)({url:'42', id: '', count: 45});
  console.log({ res });
