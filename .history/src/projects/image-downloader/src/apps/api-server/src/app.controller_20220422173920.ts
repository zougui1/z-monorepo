...
import { streamToFile, multiOptimizeImage, hashFile } from '@zougui/common.fs-utils';
import {
  downloadPage,
  Submission,
  SearchResult,
...
const saveImage = async (unprocessedMediaDoc: UnprocessedMediaDocument) => {
  ...
  try {
    ...
    const sizedOf = await sizeOf(filePath).catch(() => undefined);
    const probedSize = !sizedOf && await probe(filePath).catch(() => undefined);

    const foundSize = sizedOf || probedSize;
    const width = foundSize?.width;
    const height = foundSize?.height;
