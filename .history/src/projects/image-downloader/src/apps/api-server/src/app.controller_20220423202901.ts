...
(async () => {
  ...
  const hashesFile = '/mnt/Dev/Code/javascript/zougui/src/projects/image-downloader/src/apps/api-server/hashes.json';

  const makeHash = (): string => {
    return crypto.createHash('sha256').update(uuid.v4()).digest('hex');
  }

  console.time('get hashes');
  const hashes = await fs.pathExists(hashesFile)
    ? await fs.readJson(hashesFile)
    : new Array(5_695_077).fill(0).map(makeHash) as string[];
  await fs.writeJson(hashesFile, hashes);
  console.timeEnd('get hashes');


  const input = makeHash();

  console.time('get distances');
  const distances = hashes.map(hash => leven.get(input, hash));
  console.timeEnd('get distances');

  console.time('find close distance');
  const closeDistance = distances.find(distance => distance <= 5);
  console.timeEnd('find close distance');

  console.log('closeDistance:', closeDistance);
})();
