...
import { getErrorMessage, getErrorObject } from '@zougui/common.error-utils';
...
const saveImage = async (unprocessedMediaDoc: UnprocessedMediaDocument) => {
  ...
  try {
    ...
    console.log('image error', getErrorObject(error).message);
    await unprocessedMediaQueries.processingError(unprocessedMediaDoc._id, getErrorObject(error));
    return;
  }
