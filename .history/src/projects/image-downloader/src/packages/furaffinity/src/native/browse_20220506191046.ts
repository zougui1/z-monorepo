 ...
 */
const BrowseTask = createTaskLogs<{ args: Parameters<Browse> }, any, Error>({
  baseCode: 'furaffinity.browse',
  messages: {
    start: 'Browsing furaffinity...',
    success: ({ data }: { data: PromiseValue<ReturnType<Browse>> }) => `Found ${data.length} submissions.`,
  ...
  .dataFormatters({
    success: (submissions) => ({
      submissions: (submissions as PromiseValue<ReturnType<Browse>>).map(sub => _.pick(sub, 'url')),
    }),
