...
function ProtoProp<T>() {
  ...
  return function ProtoPropDecorator<T extends Record<string, any>>(target: T, propertyName: string) {
    ...
    Reflect.metadata(`ttype:${propertyName}`, { type, target, propertyName })(target.constructor);
  }
}
...
const getModel = (ctor: { new(...args: any[]): {} }) => {
  const properties = Reflect.getMetadata('properties', ctor) || [];
  const types = Promise.all(properties.map(async (prop: string) => {
    const ttype = Reflect.getMetadata(`ttype:${prop}`, ctor);
    ...
    const typeCtor = await type.getCtor();
