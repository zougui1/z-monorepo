...
class MediaQueries {
  ...
  hashesExist = async (hashes: string[]): Promise<{ _id: Types.ObjectId }> => {
    const media = await MediaModel.findOne({ 'file.hashes': hashes }, { _id: 1 });
    return media as { _id: Types.ObjectId };
  }
