...
class UnprocessedMediaQueries {
  ...
  findErrored = async (): Promise<UnprocessedMediaDocument[]> => {
    return UnprocessedMediaModel.find({ status: UnprocessedMediaStatus.error });
  }
  ...
  isAlreadyDownloaded = async ({ url, downloadUrl }: { url: string; downloadUrl: string }): Promise<boolean> => {
    const media = await UnprocessedMediaModel.findOne({
      $or: [
        { url },
        { downloadUrl },
      ],
