...
export const downloadSubmissions = async (search: SearchDocument): Promise<SearchResult> => {
  const erroredSubmissions = await unprocessedMediaQueries.findErrored();
  const submissionsToRetry = await Promise.all(erroredSubmissions.map(async submission => {
    return await findSubmission(submission.id);
  }));

  return await faDownloadSubmissions(submissionsToRetry);
