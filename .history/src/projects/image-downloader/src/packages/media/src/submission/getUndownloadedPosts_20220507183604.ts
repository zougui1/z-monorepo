...
export const getUndownloadedPosts = async (submissions: Submission[]): Promise<Submission[]> => {
  const submissionRequest = submissions
    .map(submission => {
      return [
        {
          metadata: { submission, type: 'processed' },
          exec: () => mediaQueries.isAlreadyDownloaded(submission),
        },
        {
          metadata: { submission, type: 'unprocessed' },
          exec: () => unprocessedMediaQueries.isAlreadyDownloaded(submission),
        },
      ];
    })
    .flat();
  ...
  require('fs').writeFileSync('/mnt/Manjaro_Data/zougui/workspace/successes.json', JSON.stringify(successes, null, 2));

  return _.uniq(successes.filter(r => !r.value).map(r => r.metadata.submission));
}
