...
import { LogKind, LogContext, LogTimings, LogType } from '@zougui/log.log-types';
...
>(
  ...
  class TaskLog {
    ...
    static #formatters: {
      start?: ((context: { data: TStartData }) => UnknownObject) | undefined;
      success?: ((context: { data: TSuccessData }) => UnknownObject) | undefined;
      error?: ((context: { cause: TErrorCause; startData: UnknownObject }) => Cause) | undefined;
    } | undefined;
    ...
    static formatters = (formatters: {
      start?: ((context: { data: TStartData }) => UnknownObject) | undefined;
      success?: ((context: { data: TSuccessData }) => UnknownObject) | undefined;
      error?: ((context: { cause: TErrorCause }) => Cause) | undefined;
    }): Constructor<TaskLog> => {
      TaskLog.#formatters = {
        ...(TaskLog.#formatters || {}),
        ...formatters,
    ...
    // TODO options to define custom loggers
    //* options.start = logger.debug (or any function of the same signature)
    //* options.success = logger.debug (or any function of the same signature)
    //* options.error = logger.debug (or any function of the same signature)
    static wrap = <TFunc extends (...args: any[]) => any>(func: TFunc): ((...args: Parameters<TFunc>) => ReturnType<TFunc>) => {
      return (...args) => {
        ...
        const res = wrapFunction<ReturnType<TFunc>>(() => func(...args), {
          ...
          onReject: cause => {
            // @ts-ignore
            const log = task.error({ cause, startData: { args } });
            logger.error(log);
            throw log.cause;
    ...
    start = (...args: LogArguments<TStartData, void>): Log<RequiredLogData<TStartData>, undefined> => {
      ...
      const outputData = inputData && TaskLog.#formatters?.start && TaskLog.#formatters.start({ data: inputData as TStartData });
    ...
    success = (...args: LogArguments<ResolvedResultData<TSuccessData>, void>): Log<RequiredLogData<TSuccessData>, undefined> => {
      const inputData = args[0]?.data;
      const outputData = inputData && TaskLog.#formatters?.success && TaskLog.#formatters.success({ data: inputData as TSuccessData });
      const actualData = outputData || inputData;
      const actualArgs = actualData ? [
        {
          ...(args[0] || {}),
    ...
    error = (...args: LogArguments<void, TErrorCause>): Log<{}, RequiredLogCause<TErrorCause>> => {
      ...
      const inputStartData = (args[0] as any)?.startData;
      const outputStartData = inputStartData && TaskLog.#formatters?.start && TaskLog.#formatters.start({ data: inputStartData as TStartData });

      const outputCause = inputCause && TaskLog.#formatters?.error && TaskLog.#formatters.error({
        cause: inputCause as TErrorCause,
        startData: outputStartData,
      });
...
> = {
  ...
  >(
    formatters: {
      start?: ((context: { data: TStartData }) => NewTStartDataOut) | undefined;
      success?: ((context: { data: TSuccessData }) => NewTSuccessDataOut) | undefined;
      error?: ((context: { cause: TErrorCause; startData: TStartDataOut }) => NewTErrorCauseOut) | undefined;
    }
  ) => TaskLogsConstructor<
    TStartData,
    TSuccessData,
